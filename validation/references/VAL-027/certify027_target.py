...
Uses accepted R4 SV-027 exact-real contact-target clarification and invariant
...
OUT=parser.parse_args().output_dir.resolve();OUT.mkdir(parents=True,exist_ok=True);start=time.monotonic()
...
inputs=['longdouble-RK4-subdivisions-64.npz','full-kernel-reference.npz','exact-residual-refined-certificate.json']
...
result={'classification':'EXACT_RATIONAL_SV027_CONTACT_TARGET_REFERENCE_CERTIFICATE',
 ...
 'limits':['R4 SV-027 establishes exact-real selected RHS contact invariance; no claim for arbitrary rounded-Q0 ODE.','Any changed post-SV009 RHS requires qualification and regeneration.','No original execution, candidate state, physical input or threshold was changed.'],
