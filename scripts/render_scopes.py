...
def documents(root=ROOT):
    ...
    for s in scopes:
        ...
        b += [f'- `{p}`' for p in s['ux']]
        b += [f'- `{p}` — C1.0-R1 selected-source restoration' for p in s.get('normative_annexes', [])]
