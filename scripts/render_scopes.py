...
def documents(root=ROOT):
    ...
    for s in scopes:
        ...
        b += ['', 'Paths ending in `/` are exclusive subtrees. Other entries are exact files. Fixture/reference/expected paths are owned as one bundle. Rerunning another owner’s fixture grants no write access. Shared tooling handoffs are enumerated in SCOPE_DEPENDENCY_GRAPH.', '', '## Forbidden changes']+[f'- {x}' for x in s['forbidden_changes']]
        ...
        b += ['', '## Specification preconditions', ('Open audit findings: '+', '.join(f'`{x}`' for x in s['issues'])+'. Stop the affected scientific path until reviewed normative consolidation resolves these. See `implementation/readiness_issues.json` and `docs/CODEX_READINESS_AUDIT.md`. This is not an invitation for Codex to choose a formula.' if s['issues'] else 'No direct specification gap recorded for this scope. Dependency acceptance is still required.'), '', 'Rollback: keep failure artifacts, revert only this scope’s unaccepted changes or abandon its unmerged branch. Never reset unrelated work or rewrite a shared fixture.','']
        docs[path]='\n'.join(b)
