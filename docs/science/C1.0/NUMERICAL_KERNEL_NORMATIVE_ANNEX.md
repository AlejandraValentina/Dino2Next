...
For cell i, J=|p(i+1)−p(i−1)|, W=|p(i+2)−p(i−2)|. Strong compression is W>0, J/min[p(i−1),p(i+1)]>.33 and u(i−1)>u(i+1). Let near(i) be the Boolean union over i−2…i+2. Apply minmod to acoustic amplitudes only where near is true; apply MC to those amplitudes elsewhere except the explicit SV-009 acoustic-extremum stencil, and apply MC to material amplitudes everywhere. BCR-S06-VERIFICATION-CONTRACT SV-009 completely specifies that sole amendment; all four differences use the same center-cell basis. The .33 threshold is inherited from the previously tested flattening family; it is not adjusted to the NP data.
