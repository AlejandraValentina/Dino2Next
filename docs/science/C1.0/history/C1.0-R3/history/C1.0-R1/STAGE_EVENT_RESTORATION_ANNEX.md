...
## ST-002 — reaction law only; integration composition remains unresolved
...
## ST-004 — preserved current-C1 selections and exact remaining omission
Current C1 selects acoustic factor 0.2, donor/thermal/sign factors 0.5, 16 retries with halving, 256-epsilon roundoff ledger, event order topology → relabel/merge → combustion, state/output defects 1e-6/1e-4, three periods and 1000-cycle failure. They remain unchanged.

H-02 remains MISSING_PREIMPLEMENTATION_SCIENTIFIC_DECISION: the exact nonlinear reaction-coordinate/SSPRK2 composition, global donor/thermal/event algorithm and full periodic state/reference-scale table are not recoverable as an adjudicated recipe. ARTIME explicitly calls its rays preparatory; ARPERIOD explicitly adjudicates no thresholds or algorithm. ARRAY supplies research polynomial code, not documentary selection. Current C1 constants do not fill these missing equations. No experimental sensitivity campaign is authorized here.


## ST-005 — restored selected ray algebra, not a complete RHS composition
