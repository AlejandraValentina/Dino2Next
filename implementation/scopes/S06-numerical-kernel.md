...
**Objective:** AR-002 reconstruction, flux, guards and SSPRK2.

**Dependencies:** S05. **Authority:** C1.0, A1.0, UX1.0 sections cited by this objective. **Allowed files:** `src/dino2next/numerics/`, `tests/numerical/kernel/`, `validation/fixtures/VAL-006/` through `VAL-011/`, `VAL-027/`. **Forbidden:** unrelated modules, normative contracts, thresholds, fixtures and scientific model choices.

Implement exact contracted behavior and stable failure codes through public typed interfaces. Create unit/contract tests plus every affected VAL adapter; references must remain independent. Acceptance: `python -m compileall src`, `python scripts/check_contracts.py`, `pytest tests/numerical/kernel validation/fixtures/VAL-006 validation/fixtures/VAL-011 validation/fixtures/VAL-027`, plus the affected gate class in VERIFICATION_EXECUTION_MATRIX when its runner exists. Preserve commands, logs, raw/derived artifacts and hashes.
