...
## Objective
Aggregate owned fixture adapters into versioned gate execution without owning their science.

## Identity
ID: `S18`. Status: `NOT_STARTED`. No scientific implementation is authorized by document existence alone.

## Dependencies
- S01 produces `ConfigSnapshot`: Hash fixture/reference resources.
- S06 produces `NumericalKernel`: Consume canonical candidate runner.
- S09 produces `CharacterizedLoss`: Read characterization provenance.
...
## Required interfaces
### ValidationRunner
- Purpose: Aggregate owned fixture adapters into versioned gate execution without owning their science.
- Inputs: GateRegistry; explicitly requested VAL IDs; immutable candidate/reference artifacts; optional preregistered experimental partitions
- Outputs/signatures: run_gate() -> ValidationResult(id,status,metrics,thresholds,artifact_hashes); aggregate() -> gate report
- Units: observable units inherited from each fixture; no cross-metric rescaling
- Owner: S18
...
- Failure semantics: GATE_UNAVAILABLE, REFERENCE_NOT_QUALIFIED, VERIFICATION_FAILURE, PARTITION_LEAKAGE
...
## Required behavior
- Import owner adapters without editing their fixture/reference/expected directories.
...
- Experimental data are never fabricated; H-03 covers missing preregistration details.
...
- `tests/unit/s18/`
- `tests/contract/s18/`
- `validation/fixtures/VAL-025/`
- `validation/references/VAL-025/`
- `validation/expected/VAL-025/`
- `tests/numerical/VAL-025/`
- `validation/fixtures/VAL-026/`
...
## Tests
- `tests/unit/s18/test_gate_aggregation.py`
