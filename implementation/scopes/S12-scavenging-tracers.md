...
## Objective
Implement two-zone state evolution, provenance relabels and scavenging outputs.

## Identity
ID: `S12`. Status: `NOT_STARTED`. No scientific implementation is authorized by document existence alone.

## Dependencies
- S04 produces `VolumeInventory`: Compose extensive primitive ledgers.
- S10 produces `EngineVolumes`: Implement the cylinder closure interface.
- S11 produces `EngineTopology`: Resolve transfer/exhaust event IDs.

## Normative IDs
`CAP-005`, `PHY-003`, `PHY-009`, `NUM-007`, `NUM-008`, `VAL-028`
...
## Required interfaces
### ScavengingModel
- Purpose: Implement two-zone state evolution, provenance relabels and scavenging outputs.
- Inputs: zone chemical masses/energy/tracers; common cylinder volume; signed transfers; chi,kappa; event occurrence
- Outputs/signatures: rates(...) -> zone rates and transfer donor state; apply_event(...) -> conserved merged/relabelled state and ledger; metrics(cycle) -> typed metrics
- Units: kg,J,K,Pa,seconds; chi/kappa/fractions dimensionless
- Owner: S12
...
- Failure semantics: ZONE_INVENTORY_INVALID, ZERO_DENOMINATOR, SCIENTIFIC_CHANGE_REQUIRED
...
## Required behavior
- Birth introduces no seed inventory and EPC merge is conservative.
- F0/F1/R/X remain separate from chemical species.
- Use selected exhaust and transfer-backflow splits without changing chi/kappa.
- H-04 must resolve missing zone-energy and metric denominator definitions; no guessed residual/trapping formula.

## Allowed paths
- `src/dino2next/scavenging/`
- `schemas/scavenging/`
- `tests/unit/s12/`
- `tests/contract/s12/`
- `validation/fixtures/VAL-028/`
- `validation/references/VAL-028/`
- `validation/expected/VAL-028/`
- `tests/numerical/VAL-028/`
...
## Tests
- `tests/unit/s12/test_birth_merge.py`
- `tests/unit/s12/test_relabel_ledger.py`
- `tests/contract/s12/test_scavenging_metrics.py`
- `tests/numerical/VAL-028/test_acceptance.py`
...
Owned numerical/experimental fixtures: VAL-028
...
python -m pytest tests/unit/s12/test_birth_merge.py tests/unit/s12/test_relabel_ledger.py tests/contract/s12/test_scavenging_metrics.py tests/numerical/VAL-028/test_acceptance.py -q
```
...
Open audit findings: `H-04`, `H-03`. Stop the affected scientific path until reviewed normative consolidation resolves these. See `implementation/readiness_issues.json` and `docs/CODEX_READINESS_AUDIT.md`. This is not an invitation for Codex to choose a formula.
