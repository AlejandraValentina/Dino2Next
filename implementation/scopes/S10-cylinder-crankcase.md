# S10 — cylinder-crankcase

## Objective
Compose the moving cylinder and crankcase balances without owning combustion or scavenging.

## Identity
ID: `S10`. Status: `NOT_STARTED`. No scientific implementation is authorized by document existence alone.

## Dependencies
- S02 produces `GeometryModel`: Get time-aligned geometry.
- S04 produces `VolumeInventory`: Update extensive state.
- S08 produces `PortPassage`: Consume paired passage transfers.

## Normative IDs
`CAP-002`, `PHY-001`, `PHY-003`, `NUM-002`, `NUM-007`, `VAL-014`
...
## Required interfaces
### EngineVolumes
- Purpose: Compose the moving cylinder and crankcase balances without owning combustion or scavenging.
- Inputs: GeometryModel; initial VolumeInventory; stage time; paired TransferLedger; explicit heat/source contributions
- Outputs/signatures: evaluate(stage) -> cylinder/crankcase rates, p*dV work ledger and VolumeState pair
- Units: m3, Pa, K, J, W, kg/s
- Owner: S10
...
- Failure semantics: INVENTORY_INADMISSIBLE, GEOMETRY_INVALID, INTERFACE_TIME_MISMATCH
...
## Required behavior
- Evaluate V and Vdot at exactly the integrator stage time.
- Cylinder and crankcase work are recorded separately with the normative sign.
- No duplication of port inventories inside 0D volumes.
- Expose a cylinder closure protocol consumed by S12; do not implement two zones twice.

## Allowed paths
- `src/dino2next/engine/volumes/`
- `tests/unit/s10/`
- `tests/contract/s10/`
...
## Tests
- `tests/unit/s10/test_moving_work.py`
- `tests/contract/s10/test_volume_port_pairing.py`
...
python -m pytest tests/unit/s10/test_moving_work.py tests/contract/s10/test_volume_port_pairing.py -q
```
