...
class AreaState:
    ...
    def __post_init__(self):
        ...
        try:recovered=[self.geometry.inverse(float(w)) for w in W]
...
class AreaHybrid(H.HybridSolver):
    def __init__(self,model,base_edges,geometry):
        super().__init__(model,base_edges);self.geometry=geometry
    ...
    def reorganize(self,state):
        ...
        for i,(left,right) in enumerate(zip(state.edges[:-1],state.edges[1:])):
            ...
            for k,(j,a,b) in enumerate(targets):
                ...
                amount=remainder.copy() if k==len(targets)-1 else state.inventory[i]*(self.geometry.segment.integral(float(a),float(b))/float(state.W[i+1]-state.W[i]))
    ...
    def sample(self,state,observation_edges):
        ...
        for j,(a,b) in enumerate(zip(obs[:-1],obs[1:])):
            volume=self.geometry.segment.integral(float(a),float(b));covered=0.
            for i,(l,r) in enumerate(zip(state.edges[:-1],state.edges[1:])):
                left,right=max(a,l),min(b,r)
                if right>left:
                    overlap=self.geometry.segment.integral(float(left),float(right))
