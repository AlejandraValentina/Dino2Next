...
iv=mp.iv;OUT=Path(__file__).resolve().parent
...
report={'author':'/root/bcr_science_review; adapted from upstream s01_reviewer qualifier','role':'Reference implementation author; requires independent review','classification':'REFERENCE_ONLY_CERTIFICATION_NOT_CANDIDATE_ACCEPTANCE','result':'PASS','method':'Composite Simpson8subintervals/cell, analytic fourth-derivative remainder, outward mp.iv80-digit arithmetic; no two-precision difference used as certificate','derivative_proof':'g_fourth/epsilon=(16z^4-48z²+12)exp(-z²)/sigma^4; z²exp(-z²)<=1,z4exp(-z²)<=1 give76/sigma4. Summed cell Simpson errors weighted by |FourierWeight|/dx<=1 yield76eps/sigma4*(dx/8)^4/180.','normalization':norm,'meshes':meshes,'half_amplitude':'Every integral, derivative bound, arithmetic enclosure and A0 scales linearly by1/2; eligibility ratios and relative qualification margins identical. No second candidate/reference execution implied.','normalization_error_in_mask':'Classification uses entireA0 interval and component magnitude interval; every101sample is strictly on one side of .01*A0.','candidate_representation_note':'Reference coefficients are C_h of exactGaussian cell averages; this certifies their reference computation. Candidate errors, phase/amplitude results and kernel gates have not been run.','mpmath_version':mp.__version__,'elapsed_seconds':time.monotonic()-start,'script_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}

# Bind portable qualification to actual installed reference/qualifier/catalogue.
import math,platform
root=OUT.parents[2]
report['status']='REFERENCE_QUALIFIED';report['candidate_imports']=False
report['source_hashes']={str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest() for p in
    [OUT/'reference.py',Path(__file__),root/'docs/science/C1.0/EXECUTABLE_VALIDATION_FIXTURES.json']}
report['environment']={'python':platform.python_version(),'mpmath':mp.__version__}
def up(value):return math.nextafter(float(value),math.inf)
A0_float=float(midpoint(A0));A0_error=up(mp.mpf(abs(A0-iv.mpf(A0_float)).b))
report['normalization'].update(stored_A0=A0_float,stored_A0_error_upper=A0_error)
for mesh in report['meshes']:
    allocation=mp.mpf(mesh['allocated_reference_error_lower'])
    for row in mesh['samples']:
        for component in row['components']:
            rb=component['real_bounds'];ib=component['imag_bounds']
            real=iv.mpf(rb);imag=iv.mpf(ib)
            cf=complex(float(midpoint(real)),float(midpoint(imag)))
            error=up(mp.mpf(abs(iv.mpc(real,imag)-iv.mpc(cf.real,cf.imag)).b))
            assert mp.mpf(error)<allocation
            component['stored_coefficient']=[cf.real,cf.imag]
            component['stored_coefficient_error_upper']=error
report['representation']='Stored float64 coefficient midpoints are enclosed including float conversion; exact epsilon/2 scaling preserves masks and relative margins.'
(OUT/'free_observation_qualification.json').write_text(json.dumps(report,indent=2)+'\n')
print(json.dumps({'status':report['status'],'meshes':len(report['meshes']),'samples_per_mesh':101,'A0':A0_float,'A0_error':A0_error,'elapsed_seconds':report['elapsed_seconds']}))
