...
def prepare(root):
    ...
    metadata={'classification':'DIRECTED_HYBRID_SHOCK_PREPARATION_NOT_EXECUTED','parameters':parameters,
              'N':80,'CFL':.2,'end_time':.0002,'guard_speed':2544,'reference_imports_candidate':False,
...
def execute(ref,metadata):
    n=80;end=.0002;m=math.ceil(n*2544*end)+4
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--reference-root',type=Path,required=True);parser.add_argument('--execute',action='store_true')
    args=parser.parse_args();ref,metadata=prepare(args.reference_root)
    if args.execute:metadata=execute(ref,metadata)
    output='shock-result.json' if args.execute else 'shock-preparation.json'
