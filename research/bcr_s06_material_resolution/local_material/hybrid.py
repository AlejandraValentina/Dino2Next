...
class HybridSolver(LocalSolver):
    ...
    def regional_options(self,state,patch):
        n=len(patch);speeds=np.zeros(n+1);regional=np.zeros((n+1,12))
        U=state.inventory/np.diff(state.edges)[:,None];w=self.recover(state)
        regional[0]=self.flux(U[0],w[0]);regional[-1]=self.flux(U[-1],w[-1])
        i=0
        while i<n:
            if not patch[i]:i+=1;continue
            end=i+1
            while end<n and patch[end]:end+=1
            # Exactly patch-touching faces plus genuine adjacent donor cells;
            # no unused regional HLLC evaluation on ordinary bulk faces.
            a,b=max(0,i-1),min(n,end+1)
            sub=State(state.edges[a:b+1],state.inventory[a:b],state.labels[a:b])
            v,f=super().rhs(sub)
            first,last=max(1,i),min(n-1,end)
            if first<=last:
                speeds[first:last+1]=v[first-a:last-a+1]
                regional[first:last+1]=f[first-a:last-a+1]
            i=end
        return speeds,regional

    def suggested_dt(self,state,cfl=.2):
        w=self.recover(state);v,_=self.regional_options(state,self.patch_cells(state))
        rate=w[:,3]+np.maximum(abs(w[:,1]-v[:-1]),abs(w[:,1]-v[1:]))
        return float(cfl*np.min(np.diff(state.edges)/rate))

    def rhs(self,state):
        v,high,_,_,_=self.rhs_options(state)
        return v,high
    ...
    def rhs_options(self,state):
        ...
        patch=self.patch_cells(state)
        speeds,regional=self.regional_options(state,patch)
        high=regional.copy();low=regional.copy()
    ...
    def _guard(self,base,initial,dt,speeds,high,low,second):
        ...
        def candidate(theta):
            start=len(self.records);status='REJECTED_CANDIDATE'
            identifier=len(self.guard_records)
            entry={'guard_candidate_id':identifier,'transaction_id':self._transaction_id,
                   'theta':theta,'second_FE':second,'selected':False}
            self.guard_records.append(entry)
            try:
                ...
                for record in self.records[start:]:
                    if isinstance(record,dict):record.update(guard_candidate_id=identifier,guard_candidate_status=status)
        def selected(theta,reductions):
            result=candidate(theta)
            self.guard_records[-1]['selected']=True
            return (*result,theta,reductions)
        try:return selected(1.,0)
        ...
        for reduction in range(9):
            try:return selected(lower,reduction)
